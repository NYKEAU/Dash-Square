...
export class Enemy {
  ...
  draw(context, mapStartX, mapStartY) {
    ...
    const angle = this.calculateDirection(
    ...
    context.translate(
      mapStartX + this.x + this.width / 2,
      mapStartY + this.y + this.height / 2
    );
  ...
  allEffectsProcessed() {
    ...
    const allParticlesProcessed = this.particles.every((particle) =>
      particle.isDone()
    );

    // Vérifier si tous les effets de coup ont été traités
    const allHitEffectsProcessed = this.hitEffects.every((hitEffect) =>
      hitEffect.isDone()
    );
  ...
  generateRandomPosition(player, mapWidth, mapHeight, attempts = 10) {
    ...
    const insideMap =
      enemyX >= 0 &&
      enemyX <= mapWidth - this.width &&
      enemyY >= 0 &&
      enemyY <= mapHeight - this.height;
