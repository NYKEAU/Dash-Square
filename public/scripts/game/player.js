...
export class Player {
  ...
  hasWeapon(weaponClass, weaponCount) {
    return this.weapons.some((weapon) => weapon instanceof weaponClass)
      ? false
      : this.weapons.length >= weaponCount
      ? null
      : true;
  }
  ...
  drawHealthBar(context) {
    const barWidth =
      this.maxHealth < this.maxBarWidth
        ? (this.health / this.maxHealth) * this.maxHealth
        : (this.health / this.maxHealth) * this.maxBarWidth; // La largeur de la b<arre d'expérience est proportionnelle à l'expérience du joueur
  ...
  drawExperienceBar(context) {
    ...
    const barWidth =
      this.maxHealth < this.maxBarWidth
        ? (this.experience / this.maxExperience) * this.maxHealth
        : (this.experience / this.maxExperience) * this.maxBarWidth; // La largeur de la barre d'expérience est proportionnelle à l'expérience du joueur
    ...
    if (this.maxHealth < this.maxBarWidth) {
      context.fillText(
        experienceText,
        barX + this.maxHealth - 2.5,
        barY + barHeight / 2 + 10
      );
    } else {
      context.fillText(
        experienceText,
        barX + this.maxBarWidth - 2.5,
        barY + barHeight / 2 + 10
      );
    }
  ...
  drawLevel(context) {
    ...
    context.fillText(
      levelText,
      (this.gameInstance.canvas.width - textWidth) / 2,
      50
    );
  ...
  drawScore(context) {
    ...
    context.fillText(
      scoreText,
      (this.gameInstance.canvas.width - textWidth) / 2,
      this.gameInstance.canvas.height - 50
    );
  ...
  getRotationAngleToEnemy() {
    ...
    if (closestEnemy && !closestEnemy.isDead) {
      const dx =
        closestEnemy.x + closestEnemy.width / 2 - (this.x + this.width / 2);
      const dy =
        closestEnemy.y + closestEnemy.height / 2 - (this.y + this.height / 2);
  ...
  removeItems() {
    ...
    this.updateStats({
      Vie: -this.health,
      VieMax: -this.maxHealth,
      Exp: -this.maxExperience,
      Argent: -this.money,
      Dégâts: -this.damage,
  ...
  increaseMoney(amount) {
    const incrementMoney = (i) => {
      if (i < amount) {
        ...
        this.drawMoney(this.gameInstance.ctx);
        setTimeout(() => incrementMoney(i + 1), 10);
      }
    };
  ...
  decreaseHealth(amount) {
    ...
    const animateHealthDecrease = () => {
      if (this.health > targetHealth) {
        ...
        this.drawHealthBar(this.gameInstance.ctx); // Utiliser le contexte de gameInstance
        requestAnimationFrame(animateHealthDecrease);
      } else {
  ...
  increaseExperience(amount) {
    ...
    const animateExperienceIncrease = () => {
      if (this.experience < targetExperience) {
        ...
        this.drawExperienceBar(this.gameInstance.ctx); // Utiliser le contexte de gameInstance
        requestAnimationFrame(animateExperienceIncrease);
      } else {
        this.experience = targetExperience;
        this.drawExperienceBar(this.gameInstance.ctx);
