...
document.addEventListener("DOMContentLoaded", async (event) => {
  ...
  if (settingsButton && settingsMenu) {
    settingsButton.addEventListener("click", () => {
      ...
      settingsButton.style.transform = isSettingsOpen
        ? "rotate(90deg)"
        : "rotate(0)";
    });
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", async (event) => {
      ...
      try {
        const data = await user.auth.login(
          pseudoInput.value,
          passwordInput.value
        );
        if (data.error) {
    ...
    forgotPasswordForm.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        if (data.error) {
          ...
          displayError(
            "Échec de la réinitialisation du mot de passe: " + data.error
          );
        } else {
        ...
        displayError(
          "Échec de la réinitialisation du mot de passe: " + error.message
        );
      }
    });
  }
  ...
  if (registerForm) {
    registerForm.addEventListener("submit", async (event) => {
      ...
      try {
        const response = await user.auth.register(
          pseudoInput.value,
          emailInput.value,
          passwordInput.value
        );
